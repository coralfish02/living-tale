---
title: "テーマを1行入れるだけで物語が育つAI「Living Tale」を作った"
emoji: "📖"
type: "idea"
...
> 「もし、キャラクターたちが語り手とは別の頭で"本音"を抱えていて、同じ場面でも解釈がズレていたら？」
...
https://www.youtube.com/watch?v=cjcSM7bUdVs
...
このうち**①ストーリー設計〜②プロット設計**の工程は、毎週膨大な時間を消費します。Living Taleは、この工程を丸ごと代替するツールではなく、**「0→1のネタ出し」——まだ何もない状態からキャラクターの動きや関係性のヒントを得る**ためのAIエージェントとして開発しました。
...
Living Taleは、このシフトの第一歩です。テーマを1行入力するだけで、語り手エージェントが物語を紡ぎ、キャラクターエージェントが独自の内心を返す。ユーザーは全員の思考を回す必要がなく、方向性を示すだけで起承転結のある物語が生まれる。生成された物語をそのまま作品にするのではなく、**「このキャラクター、こう動くのか」「この関係性、面白いかも」という0→1の気づき**を持ち帰ってもらう——noteで文章を発信する人にとってはプロットの壁打ち相手として、プロの脚本家にとってはキャラクターの化学反応を試す実験場として使えます。
...
- 🔄 **語り手エージェントが、『起』『承』『転』『結』それぞれのフェーズを段階的に創作する**ため、予定調和ではない、物語が生成される。
- 💭 **登場人物の内心が物語に奥行きを与える**——語り手エージェントとは別に、登場人物の内面を表現する「内心エージェント」を用意することで、語り手だけでは描ききれない"裏の感情"が生まれ、物語により深い奥行きが生まれる。
...
- Living Tale → **語り手（AI）が全体を見渡して舞台を描き、役者（キャラクターAI）が自分の立場だけで本音を返す「二重構造の劇」**

語り手と役者は**別々の頭**で動いている。同じ場面を見ても、全体を知る語り手と、自分の目的しか知らない役者では、解釈が違う——その"ズレ"こそが、予定調和を壊す鍵なのです。
:::
...
AIがクリエイターの仕事を奪うのではなく、**AIが「0→1のネタ出し相手」となり、まだ形のないアイデアに最初の輪郭を与える**。プロの漫画家や脚本家にとっては「このキャラ、こう動くのか」という気づきの種として、noteで文章を綴る人にとっては最初の一歩を踏み出すきっかけとして——そんな未来を、Gemini 2.0 Flashのマルチエージェント構成で実現しました。

テーマを1行入力するだけで、語り手AIが物語を紡ぎ、その裏側でキャラクターたちが自分だけの視点で本音を返す——語り手と役者の「別々の頭」が生む解釈のズレが、予定調和を超えた物語体験を生み出します。ぜひ一度、体験してみてください。
